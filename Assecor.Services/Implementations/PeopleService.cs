...
namespace Assecor.Services.Implementations
{
    public class PersonsService : IPersonsService
    {
        ...
        private readonly string fileLocationPath = Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), "Files", "DataExample.csv");

        //In memory Csv data
        public List<PersonDto> _persons { get; set; } = new List<PersonDto>();

        public PersonsService(IMapper mapper, IColorRepository colorRep, ICsvService csvService)
        {
            _mapper = mapper;
            _colorRep = colorRep;
